...
def calculate_factor_scores_and_ranks(df):
    ...
    def calculate_weighted_score(group):
        ...
        weighted_sum = sum(group.loc[group['year'] == year, 'Yearly_Score'].values[0] * weights[year] for year in range(start_year, start_year + available_years))
        return weighted_sum / sum(weights[year] for year in range(start_year, start_year + available_years))
